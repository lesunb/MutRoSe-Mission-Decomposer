...
%type <sval> reliability_option
%type <sval> reliability_val
%type <vstring> capabilities_option
...
domain_defs:	domain_defs require_def |
				...
				domain_defs rewards_def |
				domain_defs capabilities_def |
...
init_el : init_el literal {
	...
	init_el '(' FLOAT literal ')' {
		...
		//l.truth_prob = $3;
		for(string v : $4->arguments.vars) l.args.push_back(access[v]);
		init_prob.push_back(std::make_pair(l,$3));
		mdp = true;
	} |
	init_el '(' KEY_PROBABILISTIC prob_list ')' {
		init_prob_vec.push_back(*($4));
		mdp = true;
	} |
	init_el '(' '=' NAME NAME FLOAT ')' {
		init_rel.push_back(std::make_pair($4, std::make_pair($5,$6)));
		mdp = true;
	} |

prob_list : prob_list FLOAT literal  {
	$$=$1;
	assert($3->type == ATOM);
	map<string,string> access;
	// for each constant a new sort with a uniq name has been created. We access it here and retrieve its only element, the constant in questions
	...
	$$->push_back(std::make_pair(l,$2));
} | {$$ = new std::vector<std::pair<ground_literal,float>>;}
...
/////////////////////////////////////////////////////////////////////////////////////////////////////
// Reward Definition
rewards_def : '(' KEY_REWARDS rewards_def_list ')' {}
rewards_def_list : NAME-list {
	rewards_definitions = *($1);
	mdp = true;
}

/////////////////////////////////////////////////////////////////////////////////////////////////////
// Capabilities Definition
capabilities_def : '(' KEY_CAPABILITIES capabilities_def_list ')' {}
capabilities_def_list : NAME-list {
	capabilities_definitions = *($1);
...
capabilities_option: KEY_REQUIRED_CAPABILITIES '(' NAME-list ')' {$$ = $3;} | {$$ = new vector<string>();}
...
//reliability_option : KEY_RELIABILITY reliability_val {$$ = $2;} | {$$ = new std::pair<string,float>; $$->first = ""; $$->second = 1.0;} //Check if having command after | is right here
reliability_option : KEY_RELIABILITY reliability_val {$$ = $2;} | {$$ = "";}

//reliability_val : '(' NAME FLOAT ')' {$$ = new std::pair<string,float>; $$->first = $2; $$->second = $3; mdp=true;} |
//				  '(' NAME ')' {$$ = new std::pair<string,float>; $$->first=$2;$$->second = 1.0; mdp=true;}
reliability_val: '(' NAME ')' {$$ = $2; mdp=true;}
/////////////////////////////////////////////////////////////////////////////////////////////////////
// Method Definition
// @HDDL
...
effect : eff_empty {$$ = $1;}
	   ...
	   | literal {$$ = $1;}
	   | p_effect {$$ = $1;}
	   | reward_change_effect {$$ = $1;}
	   | prob_effect {$$ = $1;}

eff_empty : '(' ')' {$$ = new general_formula(); $$->type=EMPTY;}
eff_conjunction : '(' KEY_AND effect-list ')' {$$ = new general_formula(); $$->type=AND; $$->subformulae = *($3);}
eff_universal : '(' KEY_FORALL '(' typed_var_list ')' effect ')'{$$ = new general_formula(); $$->type = FORALL; $$->subformulae.push_back($6); $$->qvariables = *($4); has_forall = true;}
eff_conditional : '(' KEY_WHEN gd effect ')' {$$ = new general_formula(); $$->type=WHEN; $$->subformulae.push_back($3); $$->subformulae.push_back($4); has_when = true;}
...
/////////////////////////////////////////////////////////////////////////////////////////////////////
// Reward Changes
// @LABEL Reward changes are increasing reward values for MDP construction. The syntax follows to some extent what was
// defined in PPDDL 1.0

//atomic_predicate_instantiation : '(' NAME VAR_NAME-list-non-empty ')' {
	//$$ = new literal;
	//$$->predicate = $2;
	//for (unsigned int i = 0; i < $3->size(); i++) $$->arguments.push_back($3->at(i));
//}

reward_change_effect : '(' KEY_REWARD_CHANGE reward_op r_head r_exp ')' { $$ = new general_formula(); $$->type=REWARD_CHANGE; $$->subformulae.push_back($4); $$->subformulae.push_back($5); mdp = true;}
reward_op : KEY_INCREASE
r_head : '(' NAME ')' { $$ = new general_formula(); $$->type = REWARD; $$->predicate = $2; }
r_exp : INT { $$ = new general_formula(); $$->type = INTVALUE; $$->value = $1; $$->fvalue = 0.0; $$->predvalue = new literal; }
r_exp : FLOAT { $$ = new general_formula(); $$->type = FVALUE; $$->value = 0; $$->fvalue = $1; $$->predvalue = new literal; }
r_exp : r_pred_val { $$ = new general_formula(); $$->type = PREDVALUE; $$->value = 0; $$->fvalue = 0.0; $$->predvalue = $1; }

r_pred_val : atomic_formula { 
	$$ = new literal; 
	$$->predicate = $1->predicate;
	$$->positive = true;
	for (unsigned int i = 0; i < $1->arguments.vars.size(); i++) $$->arguments.push_back($1->arguments.vars.at(i));
}
/////////////////////////////////////////////////////////////////////////////////////////////////////
